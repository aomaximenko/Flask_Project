...
class EventLogic:
    ...
    def _validate_event(event: model.Event):
        ...
        try:
            ...
            raise LogicException(f"date format is not {DATE_FORMAT}")

    def _validate_date_unique(self, event: model.Event):
        for i in self._event_db.list():
            if event.date == i.date:
                raise LogicException(f"event for {event.date} already exists")

    def create(self, event: model.Event) -> str:
        self._validate_event(event)
        self._validate_date_unique(event)
        try:
